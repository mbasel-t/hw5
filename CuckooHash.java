 ...
 public class CuckooHash<K, V> {
      ...
      */
      public void put(K key, V value) {
         ...
         if ((table[pos] != null && table[pos].bucKey.equals(key) && table[pos].value.equals(value)) ||
             (table[pos2] != null && table[pos2].bucKey.equals(key) && table[pos2].value.equals(value))) {
                return;
            }

         /* if destination is empty, simply put. easy part is done! */
         if (table[pos] == null) {
            table[pos] = new Bucket<K, V>(key, value);

         /* if destination occupied, evict the previous value */
         } else {
            Bucket<K, V> curr = table[pos];
            table[pos] = new Bucket<K, V>(key, value);
            pos = otherHash(curr, pos);

            /*
             * if previous value's next location occupied, continue evictions
             * until a new position is found.
            */
            for(int i = 0; table[pos] != null && i < CAPACITY; i++) {
                Bucket<K, V> prev = curr;

                // swap old val for new val
                curr = table[pos];
                table[pos] = prev;
            ...
            /* 
             * verify that next location is empty. if so, place last evicted
             * value. if not, rehash and then place last evicted value.
             */
            if(table[pos] != null) {
                rehash();
                put(curr.bucKey, curr.value);
            } else {
                table[pos] = curr;
            }
         }
    ...
    private int otherHash(Bucket<K, V> bucket, int pos) {
        if(bucket != null) {
            K key = bucket.bucKey;
            int hash1 = hash1(key);
